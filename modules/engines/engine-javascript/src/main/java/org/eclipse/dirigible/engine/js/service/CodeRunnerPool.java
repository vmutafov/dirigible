...
class CodeRunnerPool {
    ...
    static CodeRunnerPoolable get() throws InterruptedException {
        Timeout timeout = new Timeout(10, TimeUnit.MINUTES);
        return pool.claim(timeout);
    }

    static class CodeRunnerAllocator implements Allocator<CodeRunnerPoolable> {
    ...
    static class CodeRunnerPoolable implements Poolable {
